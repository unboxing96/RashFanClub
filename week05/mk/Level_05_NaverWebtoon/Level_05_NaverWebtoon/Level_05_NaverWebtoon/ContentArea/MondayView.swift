...
struct MondayView: View {
    ...
    var body: some View{
        VStack(spacing:15){
            ForEach(0..<webtoonInfoMon.count/3, id:\.self){rowIndex in
                HStack(spacing: 6){
                    ForEach(0..<3){columnIndex in
                        VStack(alignment: .leading){
                            let i: Int = rowIndex * 3 + columnIndex
                            Image(webtoonInfoMon[i].imageName)
                                .resizable()
                                .aspectRatio(contentMode: .fit)
                                .frame(maxWidth: .infinity)
                            
                            Text(webtoonInfoMon[i].title)
                                .font(.system(size: 13))
                                ...
                                .truncationMode(.tail)
                            HStack{
                                Text("\(webtoonInfoMon[i].author) ★\(webtoonInfoMon[i].rate)")
                                    .font(.system(size: 8))
